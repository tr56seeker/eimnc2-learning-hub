  ...
  return (
    <PortalShell profile={profile}>
      ...
      <div className="mb-7 flex flex-wrap gap-3">
        <Link href="/teacher/exams" className="rounded-2xl border border-slate-200 bg-white/70 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300 dark:hover:bg-slate-800 active:scale-[0.97]">Back to Exams</Link>
        <Link href="/teacher/question-bank" className="rounded-2xl border border-teal-200 bg-white/70 px-4 py-2 text-sm font-semibold text-teal-700 hover:bg-teal-50 dark:border-amber-800 dark:bg-slate-900/70 dark:text-amber-400 dark:hover:bg-amber-950/40 active:scale-[0.97]">Question Bank</Link>
        <Link href={`/teacher/exams/${id}/analysis`} className="rounded-2xl border border-amber-200 bg-white/70 px-4 py-2 text-sm font-semibold text-amber-700 hover:bg-amber-50 dark:border-amber-800/50 dark:bg-slate-900/70 dark:text-amber-300 dark:hover:bg-amber-950/40 active:scale-[0.97]">Item Analysis</Link>
        <Link href={`/teacher/exams/${id}/grading`} className="rounded-2xl border border-amber-200 bg-white/70 px-4 py-2 text-sm font-semibold text-amber-700 hover:bg-amber-50 dark:border-amber-800/50 dark:bg-slate-900/70 dark:text-amber-300 dark:hover:bg-amber-950/40 active:scale-[0.97]">Grade Essays</Link>
      </div>
            ...
            return (
              <div key={row.id} className="card rounded-[1.75rem] p-6 sm:p-7">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700 dark:text-amber-400">Question {index + 1} / {question.question_type} / {questionCompetency?.code ?? "EIM"}</p>
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-slate-100 pt-6 dark:border-slate-800 md:flex-row md:items-end">
                  <form action={updateExamQuestionAction.bind(null, id, row.id)} className="grid flex-1 gap-4 md:grid-cols-3">
                    ...
                    <SubmitButton className="rounded-2xl bg-slate-950 px-5 py-3 font-semibold text-white shadow-lg shadow-slate-950/10 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.97] hover:bg-teal-700 md:self-end">Update</SubmitButton>
                  </form>
