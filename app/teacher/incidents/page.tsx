...
function riskLabel(terminated: boolean) {
  if (terminated) return { label: "High Attention", className: "border-red-200 bg-red-50 text-red-700" };
  return { label: "Needs Review", className: "border-amber-200 bg-amber-50 text-amber-800" };
}
              ...
              <div key={attempt.id} className="card rounded-[1.75rem] p-6 sm:p-7">
                <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
                  <div>
                    ...
                    <h2 className="mt-3 text-xl font-semibold text-slate-950">{learner?.full_name ?? "Learner"} &middot; {exam?.title ?? "Exam"}</h2>
                    ...
                    {attempt.termination_reason ? (
                      <p className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{attempt.termination_reason}</p>
                    ) : null}
                    ...
                    {timeline.length ? (
                      <details className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/60">
                        ...
                        <ol className="grid gap-2 border-t border-slate-200 p-4 text-sm text-slate-600">
                          {timeline.map((event, index) => (
                            <li key={index} className="flex justify-between gap-3">
                              <span>{eventLabels[event.event_type] ?? event.event_type}</span>
                              <span className="tabular-nums text-slate-400">{formatDateTime(event.created_at)}</span>
                            </li>
                          ))}
                        </ol>
                      </details>
                    ...
                    {review?.notes ? <p className="mt-3 text-sm text-slate-500">Reviewer notes: {review.notes}</p> : null}
                  </div>

                  <form action={setIncidentReviewAction.bind(null, attempt.id)} className="rounded-2xl border border-slate-200/70 bg-white/75 p-5 shadow-sm shadow-slate-200/40">
                    <label className="grid gap-2 text-sm font-semibold text-slate-700">
                      Classify Incident
                      <select name="status" defaultValue={review?.status ?? "needs_review"} className="focus-ring min-h-11 rounded-xl border border-slate-200 bg-white px-3 py-2 font-normal">
                    ...
                    <label className="mt-4 grid gap-2 text-sm font-semibold text-slate-700">
                      Reviewer Notes
                      <textarea name="notes" rows={4} defaultValue={review?.notes ?? ""} className="focus-ring rounded-xl border border-slate-200 bg-white p-3 font-normal" />
                    </label>
