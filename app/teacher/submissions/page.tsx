              ...
              <div key={latest.id} className="card rounded-[1.75rem] p-6 sm:p-7">
                <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
                  <div>
                    ...
                    <h2 className="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-100">{assignment?.title}</h2>
                      ...
                      <details className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/60 dark:border-slate-700 dark:bg-slate-800/60">
                        ...
                        <div className="grid gap-3 border-t border-slate-200 p-4 dark:border-slate-700">
                          {olderVersions.map((version, index) => (
                            <div key={version.id} className="rounded-xl border border-slate-200 bg-white p-3 text-sm dark:border-slate-700 dark:bg-slate-900">
                              ...
                              {version.content_text ? <p className="mt-1 text-slate-600 dark:text-slate-400">{version.content_text}</p> : null}
                              {version.score !== null ? (
                                <p className="mt-1 text-slate-500 dark:text-slate-400">Previous score: {version.score}/{assignment?.max_score}</p>
                              ) : null}
                              {version.feedback ? <p className="mt-1 text-slate-500 dark:text-slate-400">Feedback given: {version.feedback}</p> : null}
                            </div>
                          ))}
                        </div>
                      </details>
                  ...
                  <form action={scoreSubmissionAction} className="rounded-2xl border border-slate-200/70 bg-white/75 p-5 shadow-sm shadow-slate-200/40 dark:border-slate-700/70 dark:bg-slate-900/75 dark:shadow-black/20">
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      ...
                      <button
                        ...
                        className="rounded-2xl border border-amber-300 bg-amber-50 px-5 py-3.5 font-semibold text-amber-800 hover:bg-amber-100 dark:border-amber-800/50 dark:bg-amber-950/40 dark:text-amber-300 dark:hover:bg-amber-950/60"
                      >
                        Return for Revision
                      </button>
                    </div>
