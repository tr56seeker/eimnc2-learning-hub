...
const fieldClass = "focus-ring min-h-11 rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-2.5 text-sm font-normal text-slate-900 shadow-sm";
          ...
          {academicYears.map((year) => (
            <div key={year.id} className="card rounded-[1.5rem] p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-lg font-semibold text-slate-950">{year.year_label}</p>
                  <p className="mt-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">
                    {year.start_date ?? "No start date"} – {year.end_date ?? "No end date"}
                  </p>
                </div>
                {year.is_current ? (
                  <span className="rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-700">Current</span>
                ) : (
                  <form action={setCurrentAcademicYearAction.bind(null, year.id)}>
                    <button className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:border-teal-200 hover:text-teal-700">
                      Set as Current
                    </button>
                  </form>
                )}
              ...
              <div className="mt-5 grid gap-2">
                {(termsByYear.get(year.id) ?? []).map((term) => (
                  <div key={term.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-slate-50/60 px-4 py-3">
                    <div>
                    ...
                    {term.is_current ? (
                      ...
                      <form action={setCurrentTermAction.bind(null, term.id)}>
                        <button className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600 hover:border-teal-200 hover:text-teal-700">
                          Set as Current
                        </button>
                      </form>
                    )}
              ...
              <form action={createTermAction} className="mt-4 flex flex-wrap items-end gap-3">
                ...
                <button className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 hover:border-teal-200 hover:text-teal-700">
                  Add Term
                </button>
              </form>
            </div>
