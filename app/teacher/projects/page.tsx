          ...
          {!projects.length ? (
            ...
            <div className="grid gap-4">
              {projects.map((project) => {
                ...
                return (
                  <Link key={project.id} href={`/teacher/projects/${project.id}`} className="card block rounded-[1.5rem] p-6 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.97] hover:shadow-xl">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700 dark:text-amber-400">{competency?.code ?? "EIM"}</p>
                        <h3 className="mt-1 text-lg font-semibold text-slate-950 dark:text-slate-100">{project.title}</h3>
                      </div>
                      <span className={project.is_active !== false ? "rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-700 dark:bg-amber-950/40 dark:text-amber-400" : "rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-400"}>
