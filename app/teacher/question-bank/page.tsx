    ...
    <PortalShell profile={profile}>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="card rounded-[2rem] p-6">
          <h2 className="text-xl font-black text-slate-950">Add Question</h2>
          <div className="mt-5">
        ...
        <section className="grid gap-4">
          {!questions.length ? (
            ...
            <div key={group} className="card rounded-[2rem] p-6">
                  ...
                  <details key={question.id} className="rounded-2xl border border-white/70 bg-white/70 p-4 shadow-sm">
                    <summary className="cursor-pointer list-none">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          <p className="text-xs font-black uppercase tracking-[0.2em] text-teal-700">{question.question_type} · {question.difficulty}</p>
                          <h3 className="mt-1 font-black text-slate-950">{question.question_text}</h3>
                        </div>
                        <p className="text-sm font-bold text-slate-500">{question.points ?? 1} pt{Number(question.points ?? 1) === 1 ? "" : "s"} · {question.is_active ? "Active" : "Inactive"}</p>
                      </div>
                    </summary>
                    <div className="mt-5 grid gap-5">
                      ...
                      <form action={deleteQuestionAction.bind(null, question.id)}>
                        <ConfirmSubmitButton message="Delete this question? It cannot be used in future exams." className="rounded-2xl border border-red-200 px-5 py-3 font-black text-red-700 hover:bg-red-50">
                          Delete Question
                        </ConfirmSubmitButton>
                      </form>
                    </div>
