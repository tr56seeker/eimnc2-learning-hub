  ...
  return (
    <div className="grid gap-6">
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 dark:border-slate-700 dark:bg-slate-800/60">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-teal-700 dark:text-teal-400">Question prompt</p>
          ...
          <ol className="mt-3 grid gap-2">
            {question.choices.map((choice) => (
              <li key={"value" in choice ? `${choice.value}-${choice.label}` : `${choice.key}-${choice.text}`} className="flex gap-3 rounded-xl border border-slate-200/80 px-4 py-3 text-sm text-slate-700 dark:border-slate-700/80 dark:text-slate-300">
                <span className="font-semibold text-teal-700 dark:text-teal-400">{"value" in choice ? choice.value : choice.key}</span>
  ...
  return (
    <>
      <header className="mb-7 flex flex-col gap-5 sm:mb-9 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-700 dark:text-teal-400">Teacher</p>
      ...
      <section className="overflow-visible rounded-2xl border border-slate-200/80 bg-white shadow-sm shadow-slate-200/50 dark:border-slate-700/80 dark:bg-slate-900 dark:shadow-black/20">
        <div className="border-b border-slate-200 bg-slate-50/60 p-4 sm:p-5 dark:border-slate-700 dark:bg-slate-800/60">
          ...
          <div className="mt-3 flex items-center justify-between gap-4 text-xs text-slate-500 dark:text-slate-400">
            ...
            {hasFilters ? (
              <button type="button" onClick={clearFilters} className="font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-400 dark:hover:text-teal-300">Clear filters</button>
            ) : null}
          </div>
        </div>
        ...
        {filteredQuestions.length ? (
                  ...
                  <tr key={question.id} className="align-middle text-slate-700 hover:bg-slate-50/70 dark:text-slate-300 dark:hover:bg-slate-800/40">
                    <td className="border-b border-slate-100 px-5 py-3.5 dark:border-slate-800">
                      <button
                        ...
                        className="line-clamp-2 max-w-xl text-left font-semibold leading-5 text-slate-950 hover:text-teal-700 dark:text-slate-100 dark:hover:text-teal-400"
                    ...
                    <td className="border-b border-slate-100 px-4 py-3.5 dark:border-slate-800">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${question.isActive ? "bg-teal-50 text-teal-700 dark:bg-teal-950/40 dark:text-teal-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400"}`}>
                    ...
                    <td className="border-b border-slate-100 px-5 py-3.5 text-right dark:border-slate-800">
                      <div className="inline-flex items-center gap-1">
                        <button type="button" onClick={() => setModal({ mode: "view", question })} className="rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-teal-700 dark:text-slate-400 dark:hover:bg-slate-800/60 dark:hover:text-teal-400">View</button>
                        <button type="button" onClick={() => setModal({ mode: "edit", question })} className="rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-teal-700 dark:text-slate-400 dark:hover:bg-slate-800/60 dark:hover:text-teal-400">Edit</button>
                        <form action={deleteQuestionAction.bind(null, question.id)} onSubmit={confirmDelete}>
                          <button className="rounded-lg px-2.5 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-950/30 dark:hover:text-red-300">Delete</button>
                        </form>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="px-6 py-16 text-center">
            ...
            {questions.length && hasFilters ? (
              <button type="button" onClick={clearFilters} className="mt-3 text-sm font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-400 dark:hover:text-teal-300">Clear filters</button>
            ) : null}
          </div>
        )}
      </section>
